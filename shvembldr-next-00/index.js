...
function drawTexture (options) {
  const { texture, canvas } = options;
  canvas.image(texture, mid.x - texture.width * 0.25, mid.y - texture.height * 0.25, texture.width * 0.5, texture.height * 0.5);
}
...
function preload () {
  sidewalkTexture = loadImage('../assets/DRAW-ME.jpg');
  earthTextureMap = loadImage('../assets/frosty-grass.jpg');
}

function setup() {
  createCanvas(windowWidth, windowHeight);
  const shapesCanvas = createGraphics(windowWidth, windowHeight);
  shapesCanvas.drawingContext.filter =
  ...
  shapesCanvas2.drawingContext.filter =
  ...
  drawGrid({
    artGrid: moreArtGrid,
    shapesCanvas: shapesCanvas2,
    maskCanvas: maskCanvas2,
  });
  drawLines(getLineGrid());
  //
  // drawWedges({ wedges: getWedges(), canvas: sunburstCanvas });
  // drawWedges({ wedges: getWedges(), canvas: sunburstCanvas2 });
  drawTexture({ texture: sidewalkTexture, canvas: sunburstCanvas });
  drawTexture({ texture: sidewalkTexture, canvas: sunburstCanvas2 });
