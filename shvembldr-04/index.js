...
function drawShape(props) {
  ...
  const textureCanvas = random() > 1 ? random(textureCanvases) : null;
  if (isVisible === true) {
    if (textureCanvas === null) {
      ...
      console.log('!');
      // tempShapesCanvas.push();
      tempShapesCanvas.fill(255);
      // tempShapesCanvas.translate(x, y);
      // tempShapesCanvas.rotate(rotation);
      tempShapesCanvas.rect(x, y, size, size);
      // tempShapesCanvas[fn](x, y, size, size);
      // tempShapesCanvas.pop();
    }
  }
}
...
function getOffScreenCanvas(opts = {}) {
  const { filter } = opts;
  ...
  c.noLoop();
  if (filter) {
    c.drawingContext.filter = filter;
  }
  return c;
}
...
function setup() {
  ...
  plainCanvas = getOffScreenCanvas();
  ...
  tempShapesCanvas = getOffScreenCanvas();
  compositeCanvas = getOffScreenCanvas();
  sunburstCanvas = getOffScreenCanvas();

  canvases = [plainCanvas, dropShadowCanvas, glowCanvas];
  textureCanvases = ["red"];

  // sunburst
  mid = { x: windowWidth * 0.5, y: windowHeight * 0.5 };
  sunburstCanvas.noStroke();
  tempShapesCanvas.noStroke();
  ...
  compositeCanvas.image(tempShapesCanvas, 0, 0);
  ...
  drawLines();
  drawGrid();
  image(compositeCanvas, 0, 0); // WHY WON'T THIS RENDER, FUCKING?!
